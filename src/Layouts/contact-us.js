...
const SampleComponent = () => {
  ...
  const handleExpandClick = () => {
    ...
    useEffect(() => {
        ...
        setVetoList("veto listt", response.data.hydra);
        console.log("vetoooo list", response.data);
        const posts = response.data.results.map(obj => ({nomveterinary: obj.nomveterinary, categorieveto: obj.categorieveto}));
        this.setPosts({ posts });
		console.log("hethi data veto",{posts});
           setLoading(false);
           console.log("hethi data veto",response.data);
...
return (
   ...
   <a InputBase href="/VetoMap"    flex= '1' className="scroll banner-button">VetoMap  </a>
           ...
           {this.Posts.map(function(val, key) {
      ...
      function handleClick(e) {
        ...
        console.log("idveto",val.idVet);
        
        var idvet= val.idVet;
        var dataform = new FormData();
        dataform.append('idVet', idvet);
        console.log("idvet",idvet);
        console.log("idVetdata", dataform);
        axios({
          method:'post',
          url:'http://127.0.0.1:8000/getvetoComment',
          data: dataform,
          
      }).then( (result) => {
          console.log("res",result.data);
          setCommentveto(result.data);
...
if(handleId){
                return (
                    <div key={key}  id="unique-id" onClick={handleId}>  
                   <PerfectScrollbar> 
